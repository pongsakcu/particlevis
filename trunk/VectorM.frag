// SPHERE FRAGMENT SHADER: VELOCITY GLYPHS /W MAGNITUDE
// Use the eyespace "velocity" var to inscribe a parametric arrow onto the sphere.
...
void main (void)
{
	...
	float h = abs(dot(xy, velocity_pp)) * (w+1.0) / vel_length;
	
	// add 'arrowhead' glyph	
	//final_color = final_color - (1.0-step(0.2, h+r*0.2))*0.72;
	final_color = final_color - (1.0-step(0.1, h))*0.9;
