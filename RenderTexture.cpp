...
// Modifications by Vincent Hoon:
// Added support for multisampling, inserted variable into Initialize.
                               ...
                               UpdateMode   updateMode   /* = RT_RENDER_TO_TEXTURE */)
{
    ...
    if (RT_COPY_TO_TEXTURE == _eUpdateMode)
    {
      ...
      if (_bHasArbDepthTexture) 
      {
        ...
        /*glTexImage2D(_iTextureTarget, 0, GL_DEPTH_COMPONENT, _iWidth, _iHeight, 
			...
			glTexImage2D(_iTextureTarget, 0, GL_DEPTH_COMPONENT24, _iWidth, _iHeight, 
				0, GL_DEPTH_COMPONENT, GL_BYTE, NULL); // hax
      } 
      else 
      {
  ...
  if (bMultisample > 0)
  {
		attribChooseList[attribChoose++]=WGL_SAMPLE_BUFFERS_ARB;
		attribChooseList[attribChoose++]= GL_TRUE;

		attribChooseList[attribChoose++]=WGL_SAMPLES_ARB;
		attribChooseList[attribChoose++]=bMultisample;
  }
